  ...
  return (
    <div className="shell space-y-6 py-10">
      ...
      <div className="card p-6">
        <form className="mb-4 flex gap-2" action="/search" method="get">
          <input
            name="q"
            defaultValue={q}
          ...
          />
          <button className="btn-primary">Kerko</button>
        </form>
        <LiveSearch />
      </div>
        ...
        {[...grouped.entries()].map(([sectionName, entries]) => (
          <section key={sectionName} className="card p-5">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              {entries.map((entry) => (
                <Link key={entry.id} href={`/entries/${entry.id}`} className="rounded-xl border border-[var(--border)] bg-white p-3">
